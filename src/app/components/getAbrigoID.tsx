  ...
  useEffect(() => {
    async function fetchabrigo() {
      ...
      try {
        ...
        console.log(`Fetching abrigo with ID: ${id}`);
      } catch (error) {
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="flex flex-col items-center justify-center">
          <h1 className="font-bold">Procure por um abrigo.</h1>
          <br />
          <div>
            ...
            <input
              className="bg-blue-300 rounded-2xl border-1 border-black p-1"
              type="text"
              {...register("id")}
              id="inputid"
          ...
          <br />
          <button
            className="bg-blue-200 border-1 border-black rounded-2xl p-1"
            type="submit"
          >
            Enviar
          </button>
        ...
        {load && (
          <div key={id}>
            ...
            <p>
              Atualizado em:{" "}
              {res?.updatedAt
                ? new Date(res.updatedAt).toLocaleString("pt-BR")
                : "Data não disponível"}
            </p>
          </div>
        )}
